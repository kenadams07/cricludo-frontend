...
export async function updateTeamPlayer(
  teamId: string,
  playerId: string,
  payload: Partial<Omit<TeamPlayer, "_id" | "position">>,
) {
...
export async function reorderTeamPlayers(
  teamId: string,
  players: Pick<TeamPlayer, "_id" | "position">[],
) {
...
export async function uploadTeamFlag(teamId: string, file: File) {
  ...
  if (response?.statusCode && response.statusCode >= 400) {
    console.error("[Frontend] Upload failed:", response);
